...
> **Current state:** Phase 65 is implemented locally on `agent/phase-65-xivapi-request-deadlines` from freshly fetched merged Phase 64 `origin/main` at `f37a37b2dc118902844afc23f255ecc84f6a193f`. Every XIVAPI HTTP request now crosses one dependency-free, DOM-free boundary with a documented 15,000 ms production deadline, caller-signal composition, injected fetch/timer seams, full listener/timer cleanup, preserved caller-cancellation reasons, and a distinct timeout error. Batch timeouts classify the whole timed-out chunk without recursive bisection or individual retry, while ordinary failures retain the existing fallback and completed earlier cache writes remain intact. Manual Search, per-list/global Resolve IDs, and Regex → Item IDs retain their finally-safe producer leases, busy/action recovery, partial results, quiet/manual status behavior, Phase 64 worker isolation, and no-op dirty-state contracts. Local `npm run check` passed with 80 JavaScript files, 35 test files, and 478 tests. In-app browser QA covered ordinary lookup/scan success, controlled request timeouts and user cancellation, control/busy recovery, modal focus/background ARIA, and Comfortable/Compact layouts at 1280px, 840px, and 390px without horizontal overflow. CI and GitHub Pages were not run because implementation and publication remain separate.
