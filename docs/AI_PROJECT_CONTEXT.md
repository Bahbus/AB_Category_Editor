...
> **Current state:** Phase 61 is implemented locally on `agent/phase-61-contextual-custom-order-visibility` from freshly fetched merged Phase 60 `origin/main` at `a889aa368681c8679f723834b85e02ce2c51eb20`. `analyzeItemOrdering(...)` now exposes one non-mutating `customOrderRelevant` decision. `src/ui/itemOrderingEditor.js` appends the complete Custom Item Order section only for active Custom Order criteria, retained valid nonempty ranks, or malformed/incompatible stored data that needs Raw JSON correction; inactive omitted and valid empty values render no section, placeholder, disclosure, hidden controls, or reserved body element. Existing active, retained-inactive, malformed/corrective, lookup, validation, description, export, dirty-state, and data-fidelity behavior remains unchanged. Criterion changes and final retained-rank removal use the existing local rerender path with explicit surviving focus targets. Local `npm run check` passed with 73 JavaScript files, 32 test files, and 423 tests. Final-build in-app browser QA passed Comfortable and Compact at 1280px, 840px, and 390px with zero horizontal overflow, live appearance/disappearance, retained-rank, warning, corrective Raw JSON, and focus checks, and no console errors. CI and GitHub Pages were not run because implementation and publication remain separate.
