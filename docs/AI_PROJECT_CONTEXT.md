...
## Phase 67 implementation record

- Phase 67 established `src/locales/en.js` as a frozen, flat, plain-text English catalog and migrated the complete Preferences modal surface: title, introduction, tablist accessibility label, tab/section text, Theme/Density labels and every option/hint, both behavior checkbox labels/hints, and the saved-status message.
...
- At the Phase 67 boundary, static `index.html` chrome, empty-state Preferences guidance, Help, validation/export messages, lookup/search text, category-editor strings, generated descriptions, schema/presets, import/export, service worker, analytics, dependencies, and unrelated modules remained unchanged. Phase 68 subsequently migrated the complete About / Help and Lookup Cache modal surfaces.
...
## Phase 68 merged implementation

Phase 68 merged through PR #110 at `d53fd23f161480e7fdbd139dfdd0f1e9b2583772`.

- `src/locales/en.js` now also owns the complete About / Help and Lookup Cache modal English surfaces. Help keys are split at semantic emphasis and code boundaries so catalogs remain plain text; Lookup Cache statistics use the existing named interpolation mechanism with already locale-formatted count parameters.
- `src/app.js` injects its existing single fixed-English translator into `showHelpModal(...)` and `showLookupCacheModal(...)`. Neither modal imports localization mechanics or application orchestration, and no mutable/global locale state, persistence, selector, second catalog, or pluralization layer was added.
- `src/ui/helpModal.js` retains the exact introduction, four section headings, four lists, sixteen items, eleven `strong` runs, three `code` runs, content order, privacy claims, and shared modal behavior. Every translated value entering its HTML template is escaped; the title remains a plain-text modal sink.
- `src/ui/lookupCacheModal.js` retains `toLocaleString()` for useful/unresolved counts, passes those strings through the named `lookupCache.stats` template, escapes every translated HTML-template value, and keeps active/empty/race status updates on `textContent`. Availability, producer subscription, defensive re-checks, successful clear/close, `onClose` unsubscribe, category/cache behavior, dirty state, and focus behavior are unchanged.
- Static `index.html` launcher text, application-owned cache-clear success/refusal status, broader lookup/search, import/export, validation, category-editor, generated-description, schema, preset, startup, CSP, workflow, dependency, and unrelated UI surfaces remain unchanged. Phase 55 remains on hold.

Implementation and post-merge validation actually run:

- focused localization, application/data-flow, Help/accessibility, Lookup Cache/import/export, action-availability, cache-operation, and UI-summary coverage passed all 127 tests;
...
- fresh deployed QA verified exact Help and Lookup Cache English copy, Help semantic structure, nonempty locale-formatted cache counts, modal focus containment/return, background ARIA restoration, CSP behavior, and 840px/390px overflow behavior;
- cache clearing was deliberately not exercised against the browser profile's existing data. Direct availability, race-refusal, successful-clear, producer, cleanup, and source coverage remain authoritative for those paths;
- Phase 55 remains on hold. No locale preference, second locale, pluralization layer, static-chrome migration, broad validation/status extraction, or generated-description localization exists yet.

Confirmed future-localization limitation:

- several Help sentences currently assemble fixed-order catalog fragments around UI-owned `strong` and `code` nodes. This preserves exact English copy and semantics and is not a current English runtime bug or Phase 68 regression, but it constrains translator-controlled word order, grammar, and inflection;
- before adding a real second locale or migrating more semantically rich prose, introduce a reorderable rich-message mechanism based on complete plain-text templates with named semantic placeholders;
...
- `src/ui/helpModal.js` builds Help structure with DOM node operations. Formatter text uses `createTextNode`; semantic contents use `textContent`; and an explicit Help-local allowlist maps only `strong` and `code`. Translated rich messages never reach `innerHTML`, raw HTML parsing, attributes, URLs, handlers, or sanitizer code.
- Preferences, Lookup Cache, the single translator injection point, modal infrastructure, startup/bootstrap/CSP boundaries, static chrome, broader localization families, schema/data paths, and unrelated UI remain unchanged. No second locale, selector, locale preference, pluralization layer, dependency, or build step was added. Phase 55 remains on hold.
...
- `git diff --check origin/main` passed with no output before the durable-document update;
...
- `src/locales/en.js` now contains the exact document/brand, sidebar, and topbar English chrome strings. Neutral `action.*` keys are shared by chrome, Help, and Lookup Cache; obsolete Help-owned action labels plus the duplicate Help/Lookup Cache title keys were removed. The catalog remains frozen, string-only, and HTML-free.
- `index.html` retains the complete matching English shell as an immediate fallback, including `lang="en"`, all existing IDs/classes/roles/types/disabled states/order, and the unchanged startup/CSP/resource boundary.
- Dynamic status families, empty-state/editor/list prose, modal infrastructure/defaults, busy copy, validation/import/export messages, lookup/search list-editor text, Regex converter text, generated descriptions, locale persistence/selection, second catalogs, pluralization, schemas, presets, data paths, dependencies, styles, and Phase 55 remain unchanged.
...
Phase 71 merged through PR #116 at `ff1017ad6cb9ed1701e2ff8c851d87a0b30d6db1`. PR Project verification, CodeQL JavaScript/TypeScript and Actions analyses, and the CodeQL aggregate check passed. Post-merge Project verification run `29863854786`, Pages deployment run `29863853734`, and main-branch security scan run `29863853827` passed. Fresh post-merge `npm run check` retained the 89-file / 40-file / 512-test zero-failure baseline; `git diff --check origin/main` passed and the reviewed Phase 71 branch tree exactly matched merged `origin/main`. Fresh deployed QA confirmed Enter selection focused the connected newly rendered Equipment entry, updated the editor heading, retained `No changes`, and kept Search focus while filtering to `materia` with Equipment absent. No application warning or error appeared; Electron's generic development CSP warning was the only warning.

## Phase 72 merged state

- `src/ui/emptyState.js` now owns the complete no-category card. It builds the established card, heading, hint paragraphs, four-item list, and bundled-preset buttons with DOM node operations and receives the one application translator through `src/app.js` → `src/ui/categoryEditor.js`.
- The workflow sentence is one `emptyState.workflow.message` rich template. UI-owned placeholders create exactly two `strong` elements for shared `action.importPaste` and `action.upload` labels; translated text becomes text nodes, semantic contents use `textContent`, and the local allowlist accepts only `strong`.
- Focused `emptyState.*` keys own the title, list framing, preset labels/descriptions, manual-add text, and guidance. The existing neutral action keys remain authoritative for Import/Paste, Upload, and Preferences; exact English text, order, classes, IDs, types, accessible names, titles, and layout are unchanged.
- Both preset buttons still commit the active field and call the existing application-owned basic/advanced callbacks, which resolve preset IDs and route payloads through normal `importText(...)`. The no-category path still resets selection to `-1`; populated rendering bypasses the builder.
- The unused `label` member was removed from the two `PRESETS` records. IDs, order, `sourceLabel`, Base64 payloads, validation, import routing, replacement/dirty behavior, and preset contents remain unchanged. Localized empty-state keys are now the sole visible-label authority; status-facing `sourceLabel` remains fixed English for a later status family.
- The former hardcoded `index.html` empty-state fallback was removed so every displayed empty-state value crosses the injected translator. No locale selection/state, second catalog, status/validation family, populated list/editor copy, CSS, CSP, workflow, dependency, schema, preset data, or unrelated behavior changed.
...
- fresh local in-app browser QA verified exact empty-state text and list order, two `strong` runs, exact button IDs/types/classes/visible labels/titles/accessible names, normal basic and advanced imports producing 24 and 55 categories, populated Weapons editor rendering, `No changes` after import, Preferences/About launcher behavior, modal initial/return focus, and background inert/ARIA restoration;
- Comfortable and Compact each retained equal client/scroll widths at 1280px, 840px, and 390px. The original Comfortable density and viewport were restored and the QA tab was closed;
- the advanced preset retained its established three import warnings and normalized-display-order summary. No CSP violation or unexpected application error appeared; Electron's generic development CSP warning was the only infrastructure warning;
...
- Phase 75 is published ready for review through PR #138 with `Closes #137`; it remains unmerged and the Roadmap status remains `In Progress` while review is pending;
...
## Phase 76 current implementation

- `src/reorderMotion.js` is the one dependency-free FLIP-style boundary. It captures pre-render rectangles, matches connected replacement nodes by private motion keys, and requests a restrained 180 ms transform animation when `Element.animate(...)` is available.
- Category rows use object-reference keys, so duplicate names, IDs, and JSON-identical category objects remain distinct. Item Sort Criteria and ordered Custom Item Ranks use explicit occurrence tokens that move with the affected occurrence rather than display text or serialized value.
- Category header Move up/down, real category drops, Sort by Order, criterion movement, and custom-rank movement capture only around their existing successful reorder decisions. Invalid, no-op, and unavailable actions never start motion.
- Mutation, selection, optional renumbering, dirty state, callbacks, announcements, structural rendering, focus restoration, drag-class cleanup, validation, and accessibility updates remain immediate and authoritative.
- Reduced motion, missing animation APIs, failed setup, unchanged geometry, and disconnected replacements suppress motion without affecting the final UI. Rapid repeats cancel stale same-key animation; ordinary replacement renders cancel active reorder animation before removing nodes; completion and cancellation release bookkeeping without temporary inline styles or event listeners.
- Focused fake-node and DOM-free coverage proves occurrence identity, object-reference identity, setup, transform deltas, completion cleanup, interruption, explicit newer-render cancellation, disconnection, unsupported APIs, unchanged-position suppression, failed setup, and reduced-motion suppression. Existing source guards cover all four integrations and retain reorder/focus contracts.
- Final `npm run check` passed: 95 JavaScript files syntax-checked, all static relative imports resolved, and all 44 test files / 552 tests passed with zero failures, skips, cancellations, or todos. Final `git diff --check origin/main` passed with no output.
