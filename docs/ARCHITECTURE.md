...
That split preserves exact English copy and semantics, but several sentences are consequently assembled from fixed-order translated fragments around `strong` and `code` nodes. This is a confirmed future-localization design limitation, not a current English runtime bug or Phase 68 functional regression: fixed fragment order constrains translator-controlled word order, grammar, and inflection.

Before adding a real second locale or migrating more semantically rich prose, the leading localization candidate is a safe reorderable rich-message mechanism based on complete plain-text templates with named semantic placeholders. Catalogs must still contain no HTML, and localization mechanics must remain DOM-free. UI code should validate placeholder parts and construct text nodes plus allowlisted semantic elements without `innerHTML`, raw HTML parsing, or a sanitizer dependency. Exact current English copy and semantics must remain unchanged. Phase 68.1 records this constraint but does not design or implement the mechanism.
