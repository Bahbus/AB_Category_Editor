...
test('isUsefulLookupName accepts real lookup names and rejects sentinel labels', () => {
  ...
  assert.equal(isUsefulLookupName('(unnamed)'), false);
  assert.equal(isUsefulLookupName('unnamed'), false);
  assert.equal(isUsefulLookupName(' Unnamed '), false);
...
test('lookup helper functions use the same sentinel filtering semantics', () => {
  const cache = { Item: { 1: 'Potion', 2: '(unnamed)', 3: 'unnamed', 4: '(name unavailable)' } };
  ...
  assert.equal(hasUsefulLookupName(lookupName, 'Item', 4), false);
  assert.equal(hasUsefulLookupName(lookupName, 'Item', 5), false);
  assert.equal(lookupDisplayName('  Potion  '), 'Potion');
  assert.equal(lookupDisplayName('(unnamed)'), '');
  assert.equal(lookupDisplayName('unnamed'), '');
