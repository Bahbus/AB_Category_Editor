...
test('category editor delegates Range and State composition with narrow data flow and stable order', () => {
  ...
  assert.ok(delegation.includes('translate'));
  ...
  assert.match(filterEditor, /export function createRangeStateFilterMessages\(translate\)/);
  assert.match(filterEditor, /const messages = createRangeStateFilterMessages\(translate\)/);
  assert.doesNotMatch(filterEditor, /RANGE_FILTER_NAMES|STATE_FILTER_NAMES|filter\.label\)<\/h3>/);
  assert.match(filterEditor, /rangeFiltersSummaryParts\(rules, messages\)/);
  assert.match(filterEditor, /stateFiltersSummaryParts\(rules, messages\)/);
