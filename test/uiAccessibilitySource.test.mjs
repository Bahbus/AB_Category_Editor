...
test('generated description UI wiring stays safe and source-consistent', () => {
  ...
  assert.match(editor, /textInput\(messages\.name,[\s\S]*onBlur: \(\) => maybeAutoGenerateDescription\('name changed'\)/);
  ...
  assert.match(editor, /if \(category\.Description === generated\) setStatus\(messages\.status\.current, 'ok'\);[\s\S]*?else if \(!String\(category\.Description \|\| ''\)\.trim\(\)\) applyGeneratedDescription\(generated\);[\s\S]*?else showGenerateDescriptionConfirmation\(generated\);/);
  assert.match(editor, /replaceGeneratedDescription[\s\S]*?if \(!applyGeneratedDescription\(generated\)\) setStatus\(messages\.status\.current, 'ok'\);/);
...
test('RGB blur restores committed values and only dirties actual component changes', () => {
  ...
  const rgbBlock = source.match(/function makeRgbaNumber\(key, label, getValue, setValue\) \{(?<body>[\s\S]*?)\n  \}/)?.groups.body ?? '';
...
test('Basics and Color localization uses injected adapters and safe text or attribute sinks', () => {
  const basicEditor = read('src/ui/basicEditor.js');
  const colorEditor = read('src/ui/colorEditor.js');

  for (const source of [basicEditor, colorEditor]) {
    assert.doesNotMatch(source, /createTranslator|DEFAULT_LOCALE|localization\.js|locales\/en\.js/);
    assert.doesNotMatch(source, /innerHTML\s*=\s*translate\(/);
  }
  assert.match(basicEditor, /const messages = createBasicEditorMessages\(translate\)/);
  assert.match(basicEditor, /basicsHeading\.textContent = messages\.title/);
  assert.match(basicEditor, /escapeHtml\(messages\.confirmation\.guidance\)/);
  assert.match(basicEditor, /generateButton\.textContent = messages\.generate\.action/);
  assert.match(basicEditor, /generateButton\.title = messages\.generate\.accessible/);
  assert.match(basicEditor, /setAttribute\('aria-label', messages\.generate\.accessible\)/);
  assert.match(colorEditor, /const messages = createColorEditorMessages\(translate\)/);
  assert.match(colorEditor, /heading\.textContent = messages\.title/);
  assert.match(colorEditor, /preview\.title = messages\.previewTooltip/);
  assert.match(colorEditor, /setAttribute\('aria-label', messages\.pickerAccessible\)/);
  assert.match(colorEditor, /escapeHtml\(messages\.hex\.label\)/);
  assert.match(colorEditor, /escapeHtml\(label\)/);
  assert.match(colorEditor, /\.placeholder = messages\.hex\.placeholder/);
  assert.match(colorEditor, /setAttribute\('aria-label', messages\.alpha\.accessible\)/);
  assert.match(colorEditor, /const id = `rgba-\$\{key\}-/);
  assert.doesNotMatch(colorEditor, /rgba-\$\{label/);
});
