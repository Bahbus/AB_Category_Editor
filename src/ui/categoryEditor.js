...
export function renderEditor(deps) {
  const { getCategories, getSelectedIndex, setSelectedIndex, ensureShape, markDirty, markDirtyAndRenderList = () => markDirty({ renderList: true }), renderAll, renderList, renumberCategories, openRegexToItemIdsTool, listEditorDeps, commitActiveField = () => {}, getEditorPreferences = () => ({ autoGenerateDescriptions: false }), copyTextToClipboard = async () => false } = deps;
  ...
  const nameField = textInput('Name', cat.Name, v => updateSidebarText(() => { cat.Name = v; }, { updateHeaderTitle: true, autoGenerate: 'name changed' }), { validate: () => validateCategoryName(cat).filter(item => item.field === 'Name'), validateOnInput: true });
