...
export function renderEditor(deps) {
  const { getCategories, getSelectedIndex, setSelectedIndex, ensureShape, markDirty, markDirtyAndRenderList = () => markDirty({ renderList: true }), renderAll, renderList, renumberCategories, openRegexToItemIdsTool, listEditorDeps, commitActiveField = () => {}, getEditorPreferences = () => ({ autoGenerateDescriptions: false }) } = deps;
  ...
  function maybeAutoGenerateDescription(reason = 'rule change') {
    ...
    const generated = generateCategoryDescription(cat);
  ...
  function showGenerateDescriptionConfirmation(generated) {
    ...
    requireScopedEl(wrap, '#copyGeneratedDescription', 'generated description confirmation').onclick = async () => { await navigator.clipboard?.writeText(generated); setStatus('Generated description copied.', 'ok'); };
  ...
  const nameField = textInput('Name', cat.Name, v => updateSidebarText(() => { cat.Name = v; }, { updateHeaderTitle: true }), { validate: () => validateCategoryName(cat).filter(item => item.field === 'Name'), validateOnInput: true });
  ...
  ruleGrid.append(
    listEditor('Allowed UI Category IDs', rules.AllowedUiCategoryIds, x => {
      ...
      return Number(x);
    }, x => x, { hint: 'Game ItemUICategory row IDs accepted by this category.', lookupSheet: 'ItemUICategory', validateList: () => validateCategory(cat, cats).filter(item => item.field === 'AllowedUiCategoryIds'), onItemsChanged: () => { updateValidationUi(); maybeAutoGenerateDescription('allowed UI category IDs changed'); renderList(); }, ...listEditorDeps }),
    listEditor('Allowed Item IDs', rules.AllowedItemIds, x => {
      if (!/^-?\d+$/.test(x)) throw new Error('Item IDs must be integers.');
      return Number(x);
    }, x => x, { hint: 'Specific Item row IDs accepted by this category.', lookupSheet: 'Item', validateList: () => validateCategory(cat, cats).filter(item => item.field === 'AllowedItemIds'), onItemsChanged: () => { updateValidationUi(); maybeAutoGenerateDescription('allowed item IDs changed'); renderList(); }, ...listEditorDeps }),
  ...
  for (const filter of RANGE_FILTERS) {
    ...
    titleActions.appendChild(switchInput('Enabled', obj.Enabled, v => { obj.Enabled = v; markDirty(); setDetailsSummary(ranges, rangeFiltersSummaryParts(rules)); updateCategoryIssueSummary(); maybeAutoGenerateDescription('range filter changed'); renderList(); }));
    title.appendChild(titleActions);
    box.append(
      title,
      rangeSliderControl(filter.label, obj, () => { markDirty(); setDetailsSummary(ranges, rangeFiltersSummaryParts(rules)); updateCategoryIssueSummary(); maybeAutoGenerateDescription('range filter changed'); renderList(); }, defaults)
    );
  ...
  function renderStateFilterCard(filterName, obj) {
    ...
    titleActions.appendChild(segmentedControl(displayFilterName(filterName), obj.State ?? 0, STATE_FILTER_OPTIONS, next => {
      obj.State = next;
      markDirty();
      setDetailsSummary(bools, stateFiltersSummaryParts(rules)); updateCategoryIssueSummary(); maybeAutoGenerateDescription('state filter changed'); renderList();
    }));
    title.appendChild(titleActions);
