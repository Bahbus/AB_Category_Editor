...
export function renderEditor(deps) {
  ...
  ruleGrid.append(
    listEditor('Allowed UI Category IDs', rules.AllowedUiCategoryIds, x => {
      ...
      return value;
    }, x => x, { hint: 'Game ItemUICategory row IDs accepted by this category.', lookupSheet: 'ItemUICategory', validateList: () => validateCategory(cat, cats).filter(item => item.field === 'AllowedUiCategoryIds'), dedupeValues: true, dedupeKey: normalizeRowIdValue, onItemsChanged: () => { updateValidationUi(); maybeAutoGenerateDescription('allowed UI category IDs changed'); renderList(); }, ...listEditorDeps }),
    listEditor('Allowed Item IDs', rules.AllowedItemIds, x => {
      ...
      return value;
    }, x => x, { hint: 'Specific Item row IDs accepted by this category.', lookupSheet: 'Item', validateList: () => validateCategory(cat, cats).filter(item => item.field === 'AllowedItemIds'), dedupeValues: true, dedupeKey: normalizeRowIdValue, onItemsChanged: () => { updateValidationUi(); maybeAutoGenerateDescription('allowed item IDs changed'); renderList(); }, ...listEditorDeps }),
    patternsCard,
